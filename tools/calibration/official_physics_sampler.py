...
sys.path.insert(0, str(TOOLS_ROOT / "protocol"))
...
class OfficialPhysicsSampler(AIRobot):
    def __init__(
        ...
        self.plan_issued = 0
        self.plan_offset = plan_offset
        self.plan_stride = max(1, plan_stride)
    ...
    def planned_shot(self) -> PlannedShot:
        if self.plan:
            sample_index = self.plan_offset + self.plan_issued * self.plan_stride
            self.plan_issued += 1
...
def main() -> None:
    ...
    parser.add_argument("--plan-stride", type=int, default=1)
