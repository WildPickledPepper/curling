...
结合石壶 `localScale = (0.115, 0.115, 0.115)`：

```text
world collision radius ~= 2.5 * 0.115 / 2 = 0.14375 m
world collision height ~= 2.0 * 0.115     = 0.23 m
```

这和规则阈值里反复出现的壶半径 `0.145m` 能对上。也就是说，多壶碰撞的几何半径已经不需要猜：
Unity 资源里给出的有效半径约为 `0.14375m`，规则判断中使用 `0.145m`。
...
stone collision radius: about 0.14375m
...
`generateLastContacts()` 会处理延迟的边/顶点接触，并用 barycentric coordinate
判断 contact 是否落在已缓存的顶点/边上；如果已经由相邻三角形生成过，就丢弃重复接触。
...
convex-convex 的 face/edge 裁剪候选点生成也已经对到 PhysX 源码。现在还不足以保证
精确复现多壶碰撞后的速度/角速度转移，是因为这些通用 PhysX 模板还没有完整落成
本地可运行代码，stone-rink 的 mesh midphase/triangle cache 仍需继续工程化，
动态生成 convex MeshCollider 在 WebGL build 里的最终 cook 结果，以及
...
PhysX 源码和 wasm，convex-convex 候选点生成也已经对到源码级规则，但仍不是完整本地碰撞引擎；
...
5. 中优先级：验证或近似 Unity PhysX 的石壶-石壶碰撞；若追求更高保真，把已恢复的 convex-convex 候选点生成、mesh midphase/reduction，以及 `func70963` 的 normal/friction row 写入落成本地结构体。
...
5. 在信任本地模拟器做战术 self-play 前，补充碰撞验证样本；必要时把已恢复的 PhysX convex-convex/mesh 接触生成、`func70051` reduction 和 `func70963` 写出的 4-wide normal/friction rows 工程化实现。
...
7. 用 `tools/reverse/list_decompiled_native_calls.py` 为 PhysX native wasm 函数列出
   `f_xxx -> funcNNNNN` 调用链，方便继续追非 IL2CPP 的低层 helper。
8. 用 `tools/reverse/inspect_unity_assets.py` 检查 Unity 资源里的 tag、PhysicsManager、
   Rigidbody 和 `ExtendedColliders3D` 动态 MeshCollider 参数。
9. 用 `tools/reverse/list_icall_registrations.py` 解析 `f_vvrd(name, functionPointer)`，
   把 Unity native internal-call 注册项映射到 wasm table function。
10. 用 `tools/reverse/find_decompiled_function_by_line.py` 从巨大 `build.dcmp` 行号反查所在
   wasm-decompile 函数，方便追 PhysX 字符串引用。
11. 用 `tools/reverse/recovered_unity_random.py` 本地复刻当前 WebGL build 的
    `UnityEngine.Random`。
12. 用 `tools/reverse/resolve_physx_task_metadata.py` 从 WAT data segment 读取 PhysX
    task metadata/vtable/function-table block，并把 table index 映射回 `funcNNNNN`。
13. 用 `tools/reverse/scan_wat_function_pointer_runs.py` 扫描 WAT data segment 中的
    连续 wasm function pointer，用来找静态函数表。
14. 用 `tools/reverse/find_physx_pcm_contact_table.py` 从 WAT 线性内存中扫描 PhysX
    7x7 contact method table，定位 `g_ContactMethodTable` 与 `g_PCMContactMethodTable`。
15. 用 NVIDIA PhysX 4.1 源码交叉定位 wasm 中的 `createFinalizeMethods`、
    `createFinalizeMethods4`、`createContactPatches`、`correlatePatches` 和
    `PxFrictionType` 枚举。
16. 用 `tools/reverse/find_physx_solver_vtables.py` 辅助扫描 PhysX solver vtable
    形状；本轮用它排查了 solver 表候选，并结合函数体确认 `func70937/70941/70942/70947`
    是 Coulomb/PF 支线，不是当前 patch friction 主路径。
17. 用 PhysX 4.1 的 `DySolverConstraintsBlock.cpp` 交叉确认
    `func70917/func70919/func70920/func70921`，把 4-wide patch contact block solver
    的动态/静态迭代公式和 conclude wrapper 对回 wasm。
18. 用 `tools/reverse/physx_contact4_layout.py` 生成 WebGL 32-bit 下
...
19. 用 `tools/reverse/physx_contact_buffer_layout.py` 生成 WebGL 32-bit 下
