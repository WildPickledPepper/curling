...
For the standalone arena, first run:

```powershell
D:\anaconda3\python.exe local_arena.py start
```

The local host, port and plain connection key are `127.0.0.1`, `7788` and
`localtest`.

## Recommended Dual-Player Collector

`tools/calibration/dual_calibration_collector.py` opens two socket connections from one process,
...
```bash
python tools/calibration/dual_calibration_collector.py \
  --key "$CONNECT_KEY" \
  -H "$HOST" \
  -p "$PORT" \
  --use-reset \
  --output-file "data/calibration/local_no_sweep.jsonl" \
  --show-msg
```
...
```bash
python tools/calibration/official_physics_sampler.py \
  --key "$CONNECT_KEY" \
  -H "$HOST" \
  -p "$PORT" \
  --name SamplerA \
...
```bash
python tools/calibration/fit_physics_calibration.py \
  data/calibration/local_no_sweep.jsonl \
