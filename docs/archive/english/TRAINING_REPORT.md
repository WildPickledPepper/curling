...
Intermediate smoke, ablation, and rejected-candidate logs were pruned after
their conclusions were consolidated into this report.
...
set. A more exploratory `55%` core-candidate variant was also tested; it
underperformed and was not kept.
...
Socket smoke tests with adaptive search:
...
- `archive/legacy/mock_curling_server.py`（仅用于快速协议冒烟测试）
...
- `tools/eval/evaluate_search_distill.py`
...
```powershell
D:\anaconda3\python.exe tools/eval/evaluate_search_distill.py --games 2000 --search-games 100 --model-file model/search_distill_tactic_policy.pt --report-file log/search_distill_eval.json
```
